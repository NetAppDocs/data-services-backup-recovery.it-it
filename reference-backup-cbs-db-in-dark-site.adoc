...
summary: 'In modalità privata (senza accesso a Internet), NetApp Backup and Recovery memorizza i dati di configurazione nello stesso bucket StorageGRID o ONTAP S3 utilizzato per i backup. In caso di guasto del sistema host dell"agente Console, è possibile distribuire un nuovo agente Console e ripristinare questi dati.' 
---
...
[role="lead"]
In  (senza accesso a Internet), NetApp Backup and Recovery memorizza i dati di configurazione nello stesso bucket StorageGRID o ONTAP S3 utilizzato per i backup. In caso di guasto del sistema host dell'agente Console, è possibile distribuire un nuovo agente Console e ripristinare questi dati.

Puoi anche seguire questi passaggi per sostituire l'agente della Console e ripristinare i dati di configurazione che erano memorizzati su di esso quando usi Backup e Recovery in modalità standard on-premises, senza account cloud (ad esempio, quando usi NetApp StorageGRID come destinazione di backup).
...
Quando si installa un nuovo agente Console, scaricare la stessa versione del software dell'agente originale. Le modifiche al NetApp Backup and Recovery database possono causare l'incompatibilità delle versioni software più recenti con i vecchi backup del database. È possibile  https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html["aggiornare il software dell'agente della console alla versione più recente dopo aver ripristinato il database di backup"^].
...
. Accedi alla NetApp Console utilizzando le credenziali dell'utente admin che hai appena creato.
...
. Copia il backup di MySQL dalla posizione di backup al nuovo host dell'agente della Console. L'esempio seguente utilizza il nome file  `CBS_DB_Backup_23_05_2023.sql`.
...
----
. Assicurarsi che i volumi visualizzati siano gli stessi presenti nell'ambiente originale.
...
. Copia il file ZIP di backup del catalogo indicizzato dalla posizione di backup alla cartella  `/opt/application/netapp/cbs` sul nuovo host dell'agente della Console. Il seguente esempio usa il nome file  `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`.
...
----
. Eseguire il comando *ls* per verificare che la cartella "catalogdb1" sia stata creata e che contenga le sottocartelle `files`, `versions` e `snapshots`.
...
Aggiungi i dettagli del sistema StorageGRID associato ai tuoi sistemi ONTAP, così come sono stati configurati nella configurazione originale dell'agente Console, utilizzando il  https://docs.netapp.com/us-en/console-automation/index.html["API NetApp Console"^].
...
----
+
Mentre l'indirizzo IP, il nome utente e la password sono valori personalizzati, il nome dell'account non lo è. Il nome dell'account è sempre `account-DARKSITE1`. Inoltre, il nome utente deve usare un nome formattato come un indirizzo email.

+
Questa API restituisce una risposta simile alla seguente. Puoi recuperare il token di autorizzazione come mostrato di seguito.
...
----
+
Questa API restituisce una risposta simile alla seguente. Il valore sotto  `resourceIdentifier` denota l'_WorkingEnvironment ID_ e il valore sotto  `agentId` denota l'_x-agent-id_.
...
----
. Aggiorna il database di NetApp Backup and Recovery con i dettagli del sistema StorageGRID associato ai sistemi. Assicurati di inserire il fully qualified domain di StorageGRID, insieme alla access key e alla storage key, come mostrato di seguito:
+
[source, shell]
----
...
. Seleziona ciascun sistema ONTAP e seleziona *Visualizza backup* accanto al servizio Backup and Recovery nel pannello di destra.
+
Dovresti vedere tutti i backup creati per i tuoi volumi.

. Dalla dashboard di ripristino, nella sezione Ricerca e ripristino, seleziona *Impostazioni di indicizzazione*.
+
Assicurati che i sistemi in cui l'indicizzazione del catalogo era stata abilitata in precedenza rimangano abilitati.
