...
---
= Opzioni di pianificazione del percorso di protezione in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Oltre a questi criteri, cloud storage offre ulteriori opzioni di sicurezza se si utilizza la funzione *Datalock e protezione contro i ransomware*, nonché ulteriori risparmi sui costi selezionando classi di archiviazione a lungo termine per i file di backup meno recenti. link:prev-ontap-policy-object-options.html["Scopri di più sulla protezione DataLock e Ransomware e sulle impostazioni di archiviazione"].
