...
[role="lead"]
In NetApp Backup and Recovery, puoi eseguire il backup dei dati dei volumi dai tuoi sistemi ONTAP locali a uno storage secondario e ad Amazon S3. Usa questa pagina per preparare l'ambiente e attivare i backup.
...
+
Se si utilizza uno spazio IP diverso da "Default", potrebbe essere necessario creare una route statica per accedere allo storage a oggetti.
...
NOTE: La crittografia delle chiavi gestita dal cliente è supportata solo per Amazon FSx for NetApp ONTAP. Non è supportata per Cloud Volumes ONTAP for AWS.

Consulta  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["Configura la crittografia con chiave gestita dal cliente in AWS"^].
