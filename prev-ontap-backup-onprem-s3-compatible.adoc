...
[role="lead"]
Utilizza questa pagina per verificare che la tua configurazione sia supportata e per configurare i backup dai tuoi sistemi ONTAP locali primari verso storage a oggetti compatibile con S3 in NetApp Backup and Recovery.
...
.Passi
. Accedi alla procedura guidata Attiva backup e ripristino utilizzando uno dei seguenti metodi:
+
** Dalla pagina *Systems* della Console, seleziona il sistema e seleziona *Enable > Backup Volumes* accanto a Backup and Recovery nel pannello di destra.
...
====
.Passi
Tieni presente che se ai volumi che scegli sono già applicate policy di snapshot o di replica, le policy che selezionerai in seguito sovrascriveranno quelle esistenti.
...
TIP: Se vuoi creare una policy personalizzata prima di attivare lo snapshot, puoi usare System Manager o il comando ONTAP CLI `snapmirror policy create`. Consulta .
...
*** Inserisci il nome della policy.
*** Selezionare fino a cinque pianificazioni, in genere con frequenze diverse.
*** Per le policy di backup su oggetto, imposta le impostazioni DataLock e di protezione da ransomware. Per dettagli su DataLock e sulla protezione da ransomware, fai riferimento a link:prev-ontap-policy-object-options.html["Impostazioni dei criteri di backup su oggetto"].
*** Seleziona *Crea*.


** *Esporta gli snapshot esistenti su storage a oggetti come file di backup*: Se in questo sistema sono presenti snapshot locali per volumi che corrispondono all'etichetta di pianificazione del backup appena selezionata (ad esempio, giornaliera, settimanale, ecc.), viene visualizzato questo messaggio aggiuntivo. Seleziona questa casella per copiare tutti gli snapshot storici su storage a oggetti come file di backup e garantire la protezione più completa per i tuoi volumi.
...
. Se vuoi, seleziona la casella per *Sincronizzare automaticamente le etichette della policy di snapshot con le etichette della policy di replica e della policy di backup*. Questo crea snapshot con un'etichetta che corrisponde alle etichette nelle policy di replica e di backup. Se le policy non corrispondono, i backup non verranno creati.
...
Potresti voler visualizzare e, facoltativamente, copiare i comandi API utilizzati nella procedura guidata di attivazione di Backup and Recovery. Potresti volerlo fare per automatizzare l'attivazione del backup nei sistemi futuri.

.Passi
. Nella procedura guidata Attiva Backup and Recovery, seleziona *Visualizza richiesta API*.
