...
. Trova la macchina virtuale da cui desideri effettuare il ripristino nell'elenco e seleziona il pulsante *Ripristina macchina virtuale* per quella macchina virtuale.
+
Viene visualizzato un elenco di snapshot (punti di ripristino) creati su quella macchina virtuale.

. Seleziona la casella di controllo per lo snapshot che desideri ripristinare.
. Selezionare *Avanti*.
+
...
*** *Datastore*: immettere il nome del datastore in cui si desidera ripristinare lo snapshot.
