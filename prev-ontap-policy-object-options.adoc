...
Entrambe queste funzionalità proteggono i file di backup, garantendo sempre la disponibilità di un file di backup valido da cui ripristinare i dati in caso di tentativo di attacco ransomware ai backup. È inoltre utile per soddisfare determinati requisiti normativi che richiedono che i backup siano bloccati e conservati per un certo periodo di tempo. Quando l'opzione *Datalock and Ransomware protection* è abilitata, il bucket cloud creato come parte dell'attivazione di NetApp Backup and Recovery avrà il blocco degli oggetti e il versioning degli oggetti abilitati.
...
. Quando crei una policy, espandi la sezione *Datalock and Ransomware protection*.
