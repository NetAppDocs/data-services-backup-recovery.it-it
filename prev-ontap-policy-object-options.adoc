...
. Quando si crea un criterio, espandere la sezione *Protezione da DataLock e Ransomware*.
...
È possibile abilitare la protezione DataLock e Ransomware sui volumi ONTAP dai seguenti sistemi quando si utilizza l'archiviazione di oggetti nei seguenti provider di cloud pubblici e privati.  Nelle versioni future verranno aggiunti altri provider cloud.
...
| Cloud Volumes ONTAP in Azure | Blob di Azure endif::azure[] ifdef::gcp[] endif::gcp[] 


| Sistema ONTAP in sede | ifdef::aws[] Amazon S3 endif::aws[] ifdef::azure[] Blob di Azure endif::azure[] ifdef::gcp[] endif::gcp[] NetApp StorageGRID 
|===
