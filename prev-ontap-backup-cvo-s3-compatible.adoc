...
---
= Esegui il backup dei dati di Cloud Volumes ONTAP su storage compatibile con S3 con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Completa alcuni passaggi in NetApp Backup and Recovery per iniziare a eseguire il backup dei dati dei volumi dai tuoi sistemi Cloud Volumes ONTAP a S3.
...
Informazioni richieste per l'utilizzo di chiavi gestite dal cliente per la crittografia dei dati:: Durante la procedura guidata di attivazione, è possibile scegliere le proprie chiavi gestite dal cliente per la crittografia dei dati, anziché utilizzare le chiavi di crittografia S3 predefinite. In questo caso è necessario aver già configurato le chiavi di crittografia gestite  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["Scopri come usare le tue chiavi"^].
...
Autorizzazioni Cloud Volumes ONTAP richieste:: Quando il sistema Cloud Volumes ONTAP esegue il software ONTAP 9.12.1 o versioni successive, il ruolo IAM che fornisce le autorizzazioni a tale sistema deve includere un nuovo set di autorizzazioni compatibili con S3 specificamente per NetApp Backup e ripristino dall'ultima  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-iam-roles.html["Criterio Cloud Volumes ONTAP"^].
+
--
...
+
Se la destinazione compatibile con S3 per i backup esiste come cluster nella pagina *Sistemi*, è possibile trascinare il cluster sul sistema compatibile con S3 per avviare la setup wizard.
...
** Selezionare *Volumi* nella barra Backup e ripristino. Dalla scheda Volumi, seleziona *Azioni* image:icon-action.png["Icona Azioni"] opzione icona e seleziona *Attiva protezione 3-2-1* per un singolo volume (che non abbia già abilitato la replica o il backup su storage di oggetti).
...
. *Backup*: imposta le seguenti opzioni:
+
...
. Facoltativamente, seleziona la casella per *Correggere automaticamente le etichette non corrispondenti su snapshot, replica e backup locali*. In questo modo vengono creati snapshot con un'etichetta che corrisponde alle etichette nei criteri di snapshot, replica e backup.
