...
---
= Scopri NetApp Backup and Recovery per i volumi ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Se si prevede di eseguire una scansione di un file di backup alla ricerca di ransomware durante il processo di ripristino dei dati del volume (se si è abilitata la protezione Datalock e Ransomware per i backup nel cloud), si dovranno sostenere costi aggiuntivi per il traffico in uscita dal proprio cloud provider.
...
Se il cluster utilizza ONTAP 9.11.1 o versioni successive, puoi proteggere i backup nello storage a oggetti da eliminazione e attacchi ransomware. Ogni policy di backup prevede una sezione per _Datalock and Ransomware protection_ che può essere applicata ai file di backup per un periodo di tempo specifico, ovvero il _periodo di conservazione_.
