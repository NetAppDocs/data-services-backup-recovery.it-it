...
* <<Aggiungere credenziali per le risorse host>> per qualsiasi host Windows, Microsoft SQL Server, Oracle Database o Linux con cui NetApp Backup and Recovery deve autenticarsi. Ciò include le credenziali del sistema operativo guest utilizzate durante il ripristino di file o cartelle.
...
Crea modelli di trasformazione delle risorse per memorizzare le trasformazioni comuni che apporti alle risorse durante il loro ripristino.

.Passi
. Dal menu NetApp Backup and Recovery , selezionare *Impostazioni*.
. Seleziona la freccia rivolta verso il basso per espandere la sezione *Resource modifer template*.
...
.. Facoltativamente, è possibile immettere un gruppo di risorse, una versione e un nome.
