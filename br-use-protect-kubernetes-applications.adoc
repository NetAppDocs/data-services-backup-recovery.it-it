...
---
= Aggiungi e proteggi le applicazioni Kubernetes in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Usa l'interfaccia utente web di NetApp Backup and Recovery o i file di risorse personalizzati per aggiungere applicazioni Kubernetes da proteggere. Puoi aggiungere applicazioni basate su namespace che utilizzano risorse Kubernetes standard o applicazioni basate su VM che utilizzano una o più macchine virtuali.
...
Per proteggere un'applicazione Kubernetes, crea prima l'applicazione in NetApp Backup and Recovery. Quando crei l'applicazione, NetApp Backup and Recovery rileva l'applicazione in esecuzione sul cluster Kubernetes.
...
. In alto a destra della pagina, assicurati che *Kubernetes* sia selezionato nell'elenco dei carichi di lavoro.
. Seleziona *Visualizza* per la voce di riga del workload per visualizzare le risorse Kubernetes.
. Selezionare la scheda *Applicazioni*.
. Seleziona *Crea applicazione*.
. Inserisci un nome per l'applicazione.
. Dall'elenco *Cluster*, seleziona il cluster che ospita l'applicazione.
. Nella sezione *Filtri*, seleziona *Namespace* per filtrare le applicazioni in base al namespace.
...
+
... Seleziona *Aggiungi risorse con ambito cluster* per aggiungere risorse con ambito a livello di cluster. Se le includi, vengono aggiunte all'applicazione quando la crei.
... Facoltativamente, seleziona *Cerca* per trovare le risorse in base ai tuoi criteri di ricerca.
+

NOTE: NetApp Backup and Recovery non memorizza i parametri di ricerca o i risultati; i parametri vengono utilizzati per cercare nel cluster Kubernetes selezionato le risorse che possono essere incluse nell'applicazione.





. Backup and Recovery visualizza un elenco di risorse che corrispondono ai criteri di ricerca.
. Se l'elenco contiene le risorse che si desidera proteggere, selezionare *Avanti*.
. Facoltativamente, nell'area *Policy*, scegli una policy di protezione esistente per proteggere l'applicazione oppure creane una nuova. Se non selezioni una policy, l'applicazione viene creata senza una policy di protezione. Puoi link:br-use-policies-kubernetes-create.html#create-a-policy["aggiungere una politica di protezione"] in seguito.
...
. Inserisci un nome per l'applicazione.
. Dall'elenco *Cluster*, seleziona il cluster che ospita l'applicazione.
. Nella sezione *Filtri*, seleziona *Macchine virtuali* per creare un'applicazione basata su VM.
. Trova le macchine virtuali da aggiungere all'applicazione scegliendo uno spazio dei nomi e includendo facoltativamente i selettori di etichette.
+

NOTE: Se selezioni le VM dall'elenco, la definizione dell'applicazione è statica: le nuove VM non vengono aggiunte all'applicazione in seguito (devi modificare l'applicazione per aggiungerle e proteggerle). Se usi i selettori di etichette, non puoi scegliere singole VM o modificare l'elenco generato, ma qualsiasi VM che in seguito corrisponde al selettore viene inclusa e protetta automaticamente.

+
Le macchine virtuali selezionate vengono visualizzate nell'elenco a destra.

. Se l'elenco contiene le macchine virtuali che si desidera proteggere, selezionare *Avanti*.
. Facoltativamente, nell'area *Policy*, scegli una policy di protezione esistente per proteggere l'applicazione oppure creane una nuova. Se non selezioni una policy, l'applicazione viene creata senza una policy di protezione. Puoi link:br-use-policies-kubernetes-create.html#create-a-policy["aggiungere una politica di protezione"] in seguito.
. Nell'area *Prescript e postscript*, abilitare e configurare tutti gli hook di esecuzione prescript o postscript che si desidera eseguire prima o dopo le operazioni di backup.  Per abilitare prescript o postscript, devi averne già creato almeno unolink:br-use-manage-execution-hook-templates.html["modello di gancio di esecuzione"] .
. Seleziona *Crea*.
...
.Risultato
L'applicazione viene creata e compare nell'elenco delle applicazioni nella scheda *Applicazioni* dell'inventario di Kubernetes. NetApp Backup and Recovery consente di proteggere l'applicazione in base alle tue impostazioni, e puoi monitorare l'avanzamento nell'area *Monitoraggio*.
