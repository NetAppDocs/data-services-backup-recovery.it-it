...
[role="lead"]
In NetApp Backup and Recovery, puoi eseguire il backup dei dati dei volumi dai tuoi sistemi ONTAP locali a uno storage secondario e a Google Cloud Storage. Usa questa pagina per preparare il tuo ambiente e attivare i backup.
...
* Tieni presente che se stai utilizzando uno spazio IP diverso da quello Default, potrebbe essere necessario creare una route statica per accedere allo storage a oggetti.
...
+
Se la destinazione Google Cloud Storage per i tuoi backup esiste nella pagina *Sistemi* della Console, puoi trascinare il cluster ONTAP su Google Cloud Storage.
...
** *Chiave di crittografia*: Se hai creato un nuovo bucket di Google Cloud Storage, inserisci le informazioni relative alla chiave di crittografia che ti sono state fornite dal cloud provider. Scegli se utilizzare le chiavi di crittografia predefinite di Google Cloud oppure se scegliere le tue chiavi gestite dal cliente dal tuo account Google Cloud, per gestire la crittografia dei tuoi dati.
+

NOTE: Se hai scelto un bucket di Google Cloud Storage esistente, le informazioni di crittografia sono già disponibili, quindi non devi inserirle ora.
