...
[role="lead"]
In NetApp Backup and Recovery, è possibile creare criteri di backup VMware personalizzati che regolano la frequenza dei backup, i tempi di backup e il numero di file di backup conservati.
...
+
*** *Abilita snapshot coerenti con la VM e coerenti con l'applicazione*: Abilita questa opzione per creare snapshot coerenti con la VM e coerenti con l'applicazione. Ciò richiede che VMware Tools sia in esecuzione sulla VM. Se VMware Tools non è in esecuzione, gli snapshot saranno invece crash-consistent. Tieni presente che l'abilitazione di questa opzione può aumentare i tempi di backup e utilizzare più storage. Inoltre, la RAM attiva della VM non è inclusa negli snapshot coerenti.
