...
[role="lead"]
In NetApp Backup and Recovery, puoi eseguire il backup dei dati dei volumi dai sistemi ONTAP locali a uno storage secondario e ad Azure Blob. Usa questa pagina per preparare l'ambiente e attivare i backup.
...
+
NetApp Backup and Recovery è supportato nelle regioni Azure Government quando l'agente della Console viene distribuito nel cloud, non quando è installato nei tuoi locali. Inoltre, devi distribuire l'agente della Console da Azure Marketplace. Non puoi distribuire l'agente della Console in una regione Government dal sito web NetApp Console SaaS.
...
+
** Per le licenze PAYGO di NetApp Backup and Recovery, ti serve un abbonamento al  https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Offerta NetApp Console da Azure Marketplace"^]. La fatturazione per NetApp Backup and Recovery avviene tramite questo abbonamento.
...
+
Nota che NetApp Backup and Recovery supporta i criteri di accesso di Azure come modello di autorizzazione. Il modello di autorizzazione in base al ruolo _Azure role-based access control_ (Azure RBAC) non è attualmente supportato.
...
+
** Dalla pagina *Systems* della Console, seleziona il sistema e seleziona *Enable > Backup Volumes* accanto al servizio Backup and recovery nel pannello di destra.
+
...
+
La pagina Introduzione della procedura guidata mostra le opzioni di protezione, tra cui snapshot locali, replica e backup. Se hai scelto la seconda opzione in questo passaggio, viene visualizzata la pagina Definisci strategia di backup con un volume selezionato.
...
NOTE: Se selezioni *Bring your own bucket*, assicurati che il container che crei come destinazione di backup abbia lo stesso nome dell'account di archiviazione Azure.
