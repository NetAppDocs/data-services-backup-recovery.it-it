...
summary: Esegui il backup dei dati delle applicazioni Microsoft SQL Server dai sistemi ONTAP locali su Amazon Web Services, Microsoft Azure e StorageGRID per garantire che i tuoi dati siano protetti. 
---
...
[role="lead"]
Esegui il backup dei carichi di lavoro di Microsoft SQL Server dai sistemi ONTAP locali ad Amazon Web Services, Microsoft Azure o StorageGRID.

* Crea policy per eseguire il backup dei carichi di lavoro in base a una pianificazione e gestire le operazioni di backup e ripristino. Vedi link:br-use-policies-create.html["Creare politiche"] per le istruzioni.
* Configurare la directory di registro per gli host rilevati prima di avviare un backup.
* Crea un backup su richiesta per eseguire subito il backup dei carichi di lavoro.
...
. Seleziona gli host o i nodi su cui desideri memorizzare i log.
+
...
*Ruolo della NetApp Console richiesto* Super admin di Backup and Recovery o ruolo di backup admin di Backup and Recovery. Se stai proteggendo dati archiviati su un cluster Cloud Volumes ONTAP, è richiesto anche il ruolo Cloud Volumes ONTAP Admin. Scopri di più link:reference-roles.html["Ruoli e privilegi di backup e ripristino"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Scopri di più sui ruoli di accesso NetApp Console per tutti i servizi"^].
...
+
Se vuoi creare una policy, seleziona *Crea nuova policy* e segui le istruzioni. Vedi link:br-use-policies-create.html["Creare politiche"] per maggiori informazioni.
...
*Ruolo della NetApp Console richiesto* Super admin di Backup and Recovery o ruolo di backup admin di Backup and Recovery. Se stai proteggendo dati archiviati su un cluster Cloud Volumes ONTAP, è richiesto anche il ruolo Cloud Volumes ONTAP Admin. Scopri di più su link:reference-roles.html["Ruoli e privilegi di backup e ripristino"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Scopri di più sui ruoli di accesso NetApp Console per tutti i servizi"^].

.Passi
. Dal menu NetApp Backup and Recovery , selezionare *Inventario*.
. Selezionare un carico di lavoro per visualizzare i dettagli della protezione.
. Seleziona l'icona Azioniimage:../media/icon-action.png["Opzione Azioni"] > *Visualizza dettagli*.
. Seleziona la scheda *Gruppi di protezione*, *Gruppi di disponibilità*, *Istanze* o *Database*.
...
. Seleziona la policy da applicare al backup.
...
. Selezionare un carico di lavoro per visualizzare i dettagli della protezione.
. Seleziona l'icona Azioniimage:../media/icon-action.png["Opzione Azioni"] > *Visualizza dettagli*.
. Seleziona la scheda *Gruppi di protezione*, *Istanze* o *Database*.
