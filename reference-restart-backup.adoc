...
[role="lead"]
Puoi riavviare NetApp Backup and Recovery dall'host dell'agente della Console quando hai bisogno di ripristinare il servizio o applicare una modifica. Usa il comando che corrisponde al tuo tipo di distribuzione e al runtime del container.

L'agente Console include la funzionalità NetApp Backup and Recovery .

.Passi
. Connettiti al sistema Linux in cui è in esecuzione l'agente della Console.
+
[cols="25,50"]
|===
