...
---
= Ripristina i dati ONTAP utilizzando Cerca e ripristina in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Usa Cerca e ripristina per recuperare volumi, cartelle o file dai backup ONTAP quando non conosci il nome esatto del sistema, del volume o del file. Puoi cercare tra snapshot locali, volumi replicati e backup su storage a oggetti.
...
* Quando si ripristina un volume abilitato SnapLock, il volume ripristinato mantiene la protezione SnapLock solo se la destinazione di storage è abilitata WORM. Se si ripristina su una destinazione non-WORM, lo stato SnapLock viene rimosso dal volume ripristinato.
...
| StorageGRID | Sistema ONTAP on-premise Cloud Volumes ONTAP | Sistema ONTAP in sede 
...
** È necessario configurare un LIF dati NFS IPv4 sulla VM di archiviazione e un'esportazione NFS per il volume. Search & Restore supporta sia volumi NFS che SMB/CIFS.
** Il server RPC SnapDiff deve essere attivato sulla VM di storage. La Console lo fa automaticamente quando abiliti l'indicizzazione sul sistema. (SnapDiff è la tecnologia che identifica rapidamente le differenze di file e directory tra gli snapshot.)
...
* Requisiti di Azure:
+
** Devi registrare il provider di risorse Azure Synapse Analytics (chiamato "Microsoft.Synapse") con la tua sottoscrizione. https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["Scopri come registrare questo fornitore di risorse per il tuo abbonamento"^] Per registrare il provider di risorse, devi essere il *proprietario* o il *contributore* della sottoscrizione.
...
NOTE: Backup and Recovery non indicizza gli snapshot il cui timestamp di creazione corrisponde esattamente a quello di un altro snapshot dello stesso volume.
...
.Procedura per abilitare l'indicizzazione di un sistema:
Se l'indicizzazione è già stata abilitata per il sistema, passare alla sezione successiva per ripristinare i dati.

Innanzitutto, monta un volume separato per contenere i file di catalogo. Questo aiuta a prevenire la perdita di dati se i file di catalogo degli snapshot diventano troppo grandi. Non è necessario farlo su ogni cluster; puoi montare un volume qualsiasi da qualsiasi cluster del tuo ambiente. Se non lo fai, l'indicizzazione potrebbe non funzionare correttamente.
...
[[mount-volume-steps]]
.Procedura per montare il volume per la reindicizzazione del catalogo
...
.. Nell'area _Filtra per_, seleziona i criteri di filtro. Ad esempio, puoi selezionare il sistema in cui risiedono i dati e il tipo di file, come un file .JPEG. Puoi anche selezionare il tipo di posizione di backup se vuoi cercare solo tra gli snapshot o i file di backup disponibili nello storage a oggetti.
...
+
Tieni presente che i risultati identificano gli snapshot dei volumi locali e i volumi replicati remoti che contengono il file nella tua ricerca. Puoi scegliere di ripristinare dal file di backup nel cloud, dallo snapshot o dal volume replicato.
...
+
*** Quando ripristini da Amazon S3, seleziona l'IPspace nel cluster ONTAP in cui risiederà il volume di destinazione, inserisci la access key e la secret key per l'utente che hai creato per dare al cluster ONTAP accesso al bucket S3 e, facoltativamente, scegli un endpoint VPC privato per il trasferimento sicuro dei dati. Se usi un endpoint VPC privato, questo ha la precedenza su qualsiasi connessione proxy configurata. link:prev-ontap-backup-onprem-aws.html["Vedi i dettagli su questi requisiti"].
