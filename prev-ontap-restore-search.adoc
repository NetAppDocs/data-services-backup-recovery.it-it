...
[role="lead"]
È possibile utilizzare la funzione Cerca e ripristina per recuperare volumi, cartelle o file dai file di backup di ONTAP. La funzione Cerca e ripristina consente di effettuare ricerche in tutti i backup (inclusi snapshot locali, volumi replicati e storage a oggetti) senza dover specificare il nome esatto del sistema, del volume o del file.
