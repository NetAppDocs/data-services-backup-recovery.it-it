---
sidebar: sidebar 
permalink: br-start-discover.html 
keywords: backing up, back up, backup, backup application data, Amazon Web services, AWS, NetApp Backup and Recovery, Oracle database, Microsoft SQL Server database, SAP HANA database 
summary: Per poter utilizzare il servizio, NetApp Backup and Recovery deve prima rilevare i carichi di lavoro. 
---
= Scopri i carichi di lavoro di Microsoft SQL Server in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
Scopri i carichi di lavoro di Microsoft SQL Server in NetApp Backup and Recovery per poterli proteggere e ripristinare. Se utilizzi già SnapCenter, puoi anche importare dati e criteri di backup esistenti.

*Ruolo richiesto NetApp Console* Super admin di Backup and Recovery. Se stai proteggendo dati archiviati su un cluster Cloud Volumes ONTAP, è richiesto anche il ruolo Cloud Volumes ONTAP Admin. Scopri di più link:reference-roles.html["Ruoli e privilegi di backup e ripristino"] https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Scopri di più sui ruoli di accesso NetApp Console per tutti i servizi"^].



[[configure-dns-for-air-gapped-host-discovery-in-netapp-console-local-deployment]]
== Configura DNS per il rilevamento di host air-gapped nella distribuzione locale di NetApp Console

Se usi la distribuzione locale di NetApp Console in un ambiente isolato dalla rete, dove gli host del database non sono raggiungibili tramite DNS standard, devi modificare temporaneamente la configurazione interna di RKE2 CoreDNS. Questo permette ai pod di risolvere nomi host specifici in indirizzi IP all'interno del cluster, in modo simile all'aggiunta di voci su  `/etc/hosts` su un nodo.


IMPORTANT: Prima di apportare modifiche, controlla sia la configurazione CoreDNS esistente che l'esistente  `rke2-coredns-config.yaml` HelmChartConfig per evitare di sovrascrivere eventuali personalizzazioni già presenti. Sulla maggior parte dei nodi di distribuzione locali della NetApp Console, questo file esiste già e contiene già le richieste di risorse. Se il file esiste già, non ricrearlo; aggiungi invece solo il blocco  `servers:` a quello esistente  `valuesContent`. Applica questa modifica utilizzando i manifest RKE2 così che continui a essere gestita in modo coerente dall'operatore e dal runtime RKE2.

.Prima di iniziare
* Conserva il file HelmChartConfig sotto controllo delle modifiche, dove possibile.
* Utilizza un nome file chiaro in modo che lo scopo dell'override di CoreDNS sia evidente.
* Se il record DNS previsto non è visibile nel Corefile, controlla i log del server RKE2 e verifica la sintassi del file manifest.


.Passi
. Verifica la configurazione CoreDNS esistente per assicurarti di non sovrascrivere una modifica già presente:
+
[source, shell]
----
kubectl -n kube-system get cm rke2-coredns-rke2-coredns -o jsonpath='{.data.Corefile}'
----
. Verifica che il file HelmChartConfig esista:
+
[source, shell]
----
sudo cat /var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml
----
+
[role="tabbed-block"]
====
.Se il file esiste
--
.. Esegui il seguente script per aggiungere il  `servers:` blocco al file HelmChartConfig esistente:
+
[source, shell]
----
MANIFEST=/var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml

# 1. Back up first
sudo cp -a "$MANIFEST" "$MANIFEST.bak.$(date +%Y%m%d-%H%M%S)"

# 2. Safety checks: valuesContent must be the last key, no servers: block yet
grep -n 'servers:' "$MANIFEST"        # must return nothing
tail -c 1 "$MANIFEST" | od -c | head  # file must end with a newline

# 3. Append ONLY the servers block (nothing above it changes)
sudo tee -a "$MANIFEST" >/dev/null <<'YAML'
    servers:
    - zones:
      - zone: .
      port: 53
      plugins:
      - name: errors
      - name: health
        configBlock: |-
          lameduck 10s
      - name: ready
      - name: kubernetes
        parameters: cluster.local in-addr.arpa ip6.arpa
        configBlock: |-
          pods insecure
          fallthrough in-addr.arpa ip6.arpa
          ttl 30
      # ───────────────────────── EDIT BELOW ─────────────────────────
      # Static DNS records for hosts the corporate DNS cannot resolve.
      # Format:  <IP>  <FQDN>  [optional aliases...]
      # `fallthrough` MUST stay as the last line -- it lets every other
      # name go to the upstream resolver via the `forward` plugin below.
      - name: hosts
        configBlock: |-
          1.2.3.4 example.domain.com exampleshortname
          fallthrough
      # ───────────────────────── EDIT ABOVE ─────────────────────────
      - name: prometheus
        parameters: 0.0.0.0:9153
      - name: forward
        parameters: . /etc/resolv.conf
      - name: cache
        parameters: 30
      - name: loop
      - name: reload
      - name: loadbalance
YAML

# 4. Confirm the merged result
sudo cat "$MANIFEST"
----
.. Verifica che l'unica modifica apportata al HelmChartConfig file sia l'aggiunta del  `servers:` block.


--
.Se il file non esiste
--
.. Modifica l' `configBlock`area del seguente script e aggiungi i tuoi record DNS statici. Questo crea un file YAML HelmChartConfig nella cartella dei manifest di RKE2, che RKE2 applica automaticamente per sovrascrivere la configurazione di CoreDNS:
+
[source, shell]
----
sudo tee /var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml >/dev/null <<'YAML'
apiVersion: helm.cattle.io/v1
kind: HelmChartConfig
metadata:
  name: rke2-coredns
  namespace: kube-system
spec:
  # NOTE: This is a HelmChartConfig that *overlays* values onto the rke2-coredns
  # HelmChart already managed by RKE2. RKE2 watches /var/lib/rancher/rke2/server/manifests
  # and will re-render the rke2-coredns Helm release whenever this file changes.
  #
  # The `servers:` list below REPLACES the chart's default server definition,
  # so every plugin that was in the stock Corefile must be present here.
  # Do NOT delete plugins you don't recognize -- they are required for cluster DNS.
  valuesContent: |-
    servers:
    - zones:
      - zone: .
      port: 53
      plugins:
      - name: errors
      - name: health
        configBlock: |-
          lameduck 10s
      - name: ready
      - name: kubernetes
        parameters: cluster.local in-addr.arpa ip6.arpa
        configBlock: |-
          pods insecure
          fallthrough in-addr.arpa ip6.arpa
          ttl 30
      # ───────────────────────── EDIT BELOW ─────────────────────────
      # Static DNS records for hosts the corporate DNS cannot resolve.
      # Format:  <IP>  <FQDN>  [optional aliases...]
      # `fallthrough` MUST stay as the last line -- it lets every other
      # name go to the upstream resolver via the `forward` plugin below.
      - name: hosts
        configBlock: |-
          1.2.3.4 example.domain.com exampleshortname
          fallthrough
      # ───────────────────────── EDIT ABOVE ─────────────────────────
      - name: prometheus
        parameters: 0.0.0.0:9153
      - name: forward
        parameters: . /etc/resolv.conf
      - name: cache
        parameters: 30
      - name: loop
      - name: reload
      - name: loadbalance
YAML
sudo chmod 600 /var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml
----
.. Copia ed esegui lo script.


--
====
. Verifica che CoreDNS abbia rilevato la modifica:
+
[source, shell]
----
kubectl -n kube-system get cm rke2-coredns-rke2-coredns -o jsonpath='{.data.Corefile}'
----
+
Se la modifica non è visibile dopo alcuni minuti, riavvia l'implementazione di CoreDNS e verifica nuovamente il Corefile:

+
[source, shell]
----
kubectl -n kube-system rollout restart deploy/rke2-coredns-rke2-coredns
----
. Se devi rimuovere le voci dopo aver individuato gli host del database, fai così:
+
[role="tabbed-block"]
====
.Se hai aggiunto a un file esistente
--
.. Ripristina il file di backup oppure elimina solo il  `servers:` block all'interno del file:
+
[source, shell]
----
sudo cp -a /var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml.bak.<timestamp> \
/var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml
----
+
Non eliminare il file perché contiene la configurazione esistente delle risorse RKE2.

.. Dopo aver ripristinato il file, rimuovi le voci host. Le voci obsolete possono causare problemi di rilevamento degli host e di backup pianificato.
.. Riavvia l'implementazione di CoreDNS:
+
[source, shell]
----
kubectl -n kube-system rollout restart deploy/rke2-coredns-rke2-coredns
----


--
.Se hai creato un nuovo file
--
.. Elimina il file HelmChartConfig dalla cartella manifests:
+
[source, shell]
----
sudo rm /var/lib/rancher/rke2/server/manifests/rke2-coredns-config.yaml
----
.. Dopo aver eliminato il file, rimuovi le voci host. Le voci obsolete possono causare problemi di rilevamento degli host e di backup pianificato.
.. Riavvia l'implementazione di CoreDNS:
+
[source, shell]
----
kubectl -n kube-system rollout restart deploy/rke2-coredns-rke2-coredns
----


--
====




[[discover-microsoft-sql-server-workloads-and-optionally-import-snapcenter-resources]]
== Scopri i carichi di lavoro di Microsoft SQL Server e, facoltativamente, importa le risorse SnapCenter

Durante la fase di rilevamento, NetApp Backup and Recovery analizza le istanze e i database di Microsoft SQL Server presenti sui sistemi della tua organizzazione.

Il servizio valuta i livelli di protezione esistenti, comprese le attuali policy di backup, le Snapshot e le opzioni di backup e recovery.

La scoperta avviene nei seguenti modi:

* Se hai già SnapCenter, importa le risorse SnapCenter in NetApp Backup and Recovery utilizzando l'interfaccia utente NetApp Backup and Recovery .
+

NOTE: Se hai già SnapCenter, prima controlla di aver soddisfatto i prerequisiti prima di importare da SnapCenter. Ad esempio, dovresti aggiungere i sistemi storage cluster locali di SnapCenter alla NetApp Console prima di importare da SnapCenter. Vedi link:concept-start-prereq-snapcenter-import.html["Prerequisiti per l'importazione di risorse da SnapCenter"].

* Se non hai già SnapCenter, puoi comunque individuare i carichi di lavoro aggiungendo un vCenter manualmente ed eseguire l'individuazione.




[[if-snapcenter-is-already-installed-import-snapcenter-resources-into-netapp-backup-and-recovery]]
=== Se SnapCenter è già installato, importare le risorse SnapCenter in NetApp Backup and Recovery

Se SnapCenter è già installato, importare le risorse SnapCenter in NetApp Backup and Recovery seguendo questi passaggi.  NetApp Console rileva risorse, host, credenziali e pianificazioni da SnapCenter; non è necessario ricreare tutte queste informazioni.

Puoi farlo nei seguenti modi:

* Durante la scoperta, seleziona un'opzione per importare risorse da SnapCenter.
* Dopo l'individuazione, dalla pagina Inventario, seleziona un'opzione per importare le risorse SnapCenter .
* Dopo l'individuazione, dal menu Impostazioni, seleziona un'opzione per importare le risorse SnapCenter .  Per i dettagli, vederelink:br-start-configure.html["Configurare NetApp Backup and Recovery"] .


Si tratta di un processo in due fasi:

* Importa l'applicazione SnapCenter Server e le risorse host
* Gestisci le risorse host SnapCenter selezionate




==== Importa l'applicazione SnapCenter Server e le risorse host

Questo primo passaggio importa le risorse host da SnapCenter e le visualizza nella pagina Inventario NetApp Backup and Recovery .  A quel punto, le risorse non sono ancora gestite da NetApp Backup and Recovery.


TIP: Dopo aver importato le risorse host SnapCenter , NetApp Backup and Recovery non assume automaticamente la gestione della protezione.  Per farlo, è necessario selezionare esplicitamente la gestione delle risorse importate in NetApp Backup and Recovery.  In questo modo sarai pronto a sottoporre tali risorse a backup tramite NetApp Backup and Recovery.

.Passi
. Dal menu di navigazione a sinistra NetApp Console , selezionare *Protezione* > *Backup e ripristino*.
. Selezionare *Inventario*.
. Seleziona *Scopri risorse*.
. Dalla pagina delle risorse del carico di lavoro NetApp Backup and Recovery Discover, seleziona *Importa da SnapCenter*.
. Inserisci * credenziali dell'applicazione SnapCenter *:
+
.. * FQDN o indirizzo IP SnapCenter *: immettere il FQDN o l'indirizzo IP dell'applicazione SnapCenter stessa.
.. *Porta*: immettere il numero di porta per il server SnapCenter .
.. *Nome utente* e *Password*: immettere il nome utente e la password per il server SnapCenter .
.. *Agente console*: seleziona l'agente console per SnapCenter.


. Inserisci * credenziali dell'host del server SnapCenter *:
+
.. *Credenziali esistenti*: se selezioni questa opzione, puoi utilizzare le credenziali esistenti che hai già aggiunto.  Scegli il nome delle credenziali.
.. *Aggiungi nuove credenziali*: se non disponi di credenziali host SnapCenter esistenti, puoi aggiungerne di nuove. Immettere il nome delle credenziali, la modalità di autenticazione, il nome utente e la password.


. Selezionare *Importa* per convalidare le voci e registrare SnapCenter Server.
+

NOTE: Se il SnapCenter Server è già registrato, è possibile aggiornare i dati di registrazione esistenti.



.Risultato
La pagina Inventario mostra le risorse SnapCenter importate che includono host, istanze e database di Microsoft SQL Server.

Per visualizzare i dettagli delle risorse SnapCenter importate, selezionare l'opzione *Visualizza dettagli* dal menu Azioni.



==== Gestire le risorse host SnapCenter

Dopo aver importato le risorse di SnapCenter, gestisci le risorse host in NetApp Backup and Recovery. Dopo che scegli di gestirle, NetApp Backup and Recovery può eseguire il backup e il ripristino delle risorse importate. Non gestisci più queste risorse in SnapCenter Server.

.Passi
. Dopo aver importato le risorse SnapCenter , dal menu Backup e ripristino, selezionare *Inventario*.
. Dalla pagina Inventario, seleziona l'host SnapCenter importato che vuoi che NetApp Backup and Recovery gestisca.
. Seleziona l'icona Azioniimage:../media/icon-action.png["Opzione Azioni"] > *Visualizza dettagli* per visualizzare i dettagli del carico di lavoro.
. Dalla pagina Inventario > carico di lavoro, seleziona l'icona Azioniimage:../media/icon-action.png["Opzione Azioni"] > *Gestisci* per visualizzare la pagina Gestisci host.
. Selezionare *Gestisci*.
. Nella pagina Gestisci host, seleziona se utilizzare un vCenter esistente o aggiungerne uno nuovo.
. Selezionare *Gestisci*.
+
La pagina Inventario mostra le risorse SnapCenter appena gestite.



Facoltativamente, è possibile creare un report delle risorse gestite selezionando l'opzione *Genera report* dal menu Azioni.



==== Importare le risorse SnapCenter dopo la scoperta dalla pagina Inventario

Se hai già scoperto delle risorse, puoi importare le risorse SnapCenter dalla pagina Inventario.

.Passi
. Dal menu di navigazione a sinistra della Console, seleziona *Protezione* > *Backup e ripristino*.
. Selezionare *Inventario*.
. Dalla pagina Inventario, seleziona *Importa risorse SnapCenter *.
. Per importare le risorse SnapCenter , seguire i passaggi descritti nella sezione *Importa risorse SnapCenter * sopra.




[[if-you-dont-have-snapcenter-installed-add-a-vcenter-and-discover-resources]]
=== Se SnapCenter non è installato, aggiungi un vCenter e scopri le risorse

Se non hai già installato SnapCenter, puoi aggiungere le informazioni di vCenter e permettere a NetApp Backup and Recovery di individuare i carichi di lavoro. All'interno di ciascun agente della Console, seleziona i sistemi in cui vuoi individuare i carichi di lavoro.

Questo passaggio è facoltativo se hai un ambiente VMware.

.Passi
. Dal menu di navigazione a sinistra della Console, seleziona *Protezione* > *Backup e ripristino*.
+
Se si accede a Backup and Recovery per la prima volta e si ha un sistema nella Console ma non sono state rilevate risorse, viene visualizzata la pagina _Benvenuti nel nuovo NetApp Backup and Recovery_ con l'opzione *Rileva risorse*.

. Seleziona *Scopri risorse*.
. Inserisci le seguenti informazioni:
+
.. *Tipo di carico di lavoro*: per questa versione è disponibile solo Microsoft SQL Server.
.. *Impostazioni vCenter*: seleziona un vCenter esistente o aggiungine uno nuovo. Per aggiungere un nuovo vCenter, immettere l'FQDN o l'indirizzo IP del vCenter, il nome utente, la password, la porta e il protocollo.
+

TIP: Se stai inserendo informazioni su vCenter, inserisci le informazioni sia per le impostazioni di vCenter sia per la registrazione dell'host. Se hai aggiunto o inserito informazioni su vCenter qui, devi anche aggiungere le informazioni del plug-in nelle Impostazioni avanzate.

.. *Registrazione host*: seleziona *Aggiungi credenziali* e inserisci le informazioni sugli host che contengono i carichi di lavoro che desideri scoprire.
+

TIP: Se si aggiunge un server autonomo e non un server vCenter, immettere solo le informazioni sull'host.



. Seleziona *Scopri*.
+

TIP: Questo processo potrebbe richiedere alcuni minuti.

. Continua con Impostazioni avanzate.




==== Imposta le opzioni delle impostazioni avanzate durante la fase di rilevamento e installa il plug-in

Con Impostazioni avanzate, puoi installare manualmente l'agente del plug-in su tutti i server che stai registrando. Questo ti permette di importare tutti i carichi di lavoro SnapCenter in NetApp Backup and Recovery così puoi gestire backup e ripristini da lì. NetApp Backup and Recovery mostra i passaggi necessari per installare il plug-in.

.Passi
. Dalla pagina Scopri le risorse, vai alle Impostazioni avanzate selezionando la freccia rivolta verso il basso a destra.
. Nella pagina Scopri le risorse del carico di lavoro, immetti le seguenti informazioni.
+
** *Inserisci il numero di porta del plug-in*: Inserisci il numero di porta che il plug-in utilizza.
** *Percorso di installazione*: Inserisci il percorso in cui verrà installato il plug-in.


. Se vuoi installare l'agente SnapCenter manualmente, seleziona le seguenti opzioni:
+
** *Usa l'installazione manuale*: Seleziona questa opzione per installare il plug-in manualmente.
** *Aggiungi tutti gli host del cluster*: Seleziona questa opzione per aggiungere tutti gli host del cluster a NetApp Backup and Recovery durante la fase di rilevamento.
** *Salta i controlli preliminari facoltativi*: Seleziona questa opzione per saltare i controlli preliminari facoltativi. Potresti volerlo fare, ad esempio, se sai che le esigenze di memoria o di spazio cambieranno a breve e vuoi installare il plug-in ora.


. Seleziona *Scopri*.




==== Continua alla dashboard NetApp Backup and Recovery

. Dal menu NetApp Console, seleziona *Protezione* > *Backup and Recovery*.
. Selezionare un riquadro del carico di lavoro (ad esempio, Microsoft SQL Server).
. Dal menu Backup e ripristino, seleziona *Dashboard*.
. Verifica lo stato di protezione dei dati. Il numero di carichi di lavoro a rischio o protetti aumenta in base ai carichi di lavoro appena scoperti, protetti e sottoposti a backup.
+
link:br-use-dashboard.html["Scopri cosa ti mostra la Dashboard"].


