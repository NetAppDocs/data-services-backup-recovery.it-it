...
---
= Esegui il backup degli archivi dati su Google Cloud Platform con NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
È possibile eseguire il backup di uno o più datastore su Google Cloud Platform integrando il SnapCenter Plug-in for VMware vSphere con NetApp Backup and Recovery.  Ciò aiuterà gli amministratori delle VM a eseguire facilmente e rapidamente il backup e l'archiviazione dei dati per aumentare l'efficienza di archiviazione e accelerare la transizione al cloud.
...
. Seleziona *Google Cloud Platform* per configurarlo come provider cloud.
+
.. Seleziona il progetto Google Cloud in cui desideri creare il bucket Google Cloud Storage per i backup.
.. Nel campo Chiave di accesso Google Cloud, specificare la chiave.
.. Nel campo Chiave segreta di Google Cloud, specifica la password.
