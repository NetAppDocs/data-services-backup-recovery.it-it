...
---
= Differenze di policy tra SnapCenter e NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Utilizza questo documento di riferimento per comprendere le differenze di policy tra SnapCenter e NetApp Backup and Recovery che possono influire sulle risorse e sulle policy importate.
...
NetApp Backup and Recovery utilizza i seguenti livelli di pianificazione, che differiscono leggermente:
...
NetApp Backup and Recovery regola le pianificazioni di SnapCenter come segue:
...
* Se la pianificazione di SnapCenter è impostata su un intervallo superiore a 7 giorni, NetApp Backup and Recovery imposta la pianificazione su mensile. Alcuni snapshot verranno saltati durante il mese.
+
...
*Esempio*: Se si dispone di una policy oraria SnapCenter con un intervallo di ripetizione ogni 5 ore a partire dall'1:00 AM, NetApp Backup and Recovery (dopo la migrazione) imposterà la pianificazione su giornaliera con intervalli di 5 ore all'1:00 AM, 6:00 AM, 11:00 AM, 16:00 (4:00 PM) e 21:00 (9:00 PM). Alcune ore verranno saltate — dopo le 21:00 dovrebbe essere le 2:00 AM per ripetersi ogni 5 ore, ma sarà sempre l'1:00 AM.
