...
[role="lead"]
Utilizza questa panoramica per scoprire come NetApp Backup and Recovery protegge i tuoi carichi di lavoro ONTAP, quali carichi di lavoro e destinazioni di backup supporta e come funzionano le licenze e i costi.
...
Il supporto per il backup e il ripristino è già integrato in tutti i sistemi ONTAP, quindi non sono necessari hardware o gateway multimediali aggiuntivi. Questo rende le operazioni di backup semplici ed economiche. NetApp Console semplifica l'implementazione di qualsiasi strategia di backup, incluse tutte le varianti di backup 3-2-1, senza la necessità di più gestori di risorse o personale specializzato.
...
** Esegui il backup dei carichi di lavoro VMware su provider di storage supportati.
...
** Fare riferimento a link:br-use-oracle-protect-overview.html["Proteggi i carichi di lavoro Oracle Database"].
...
** In Google Cloud, viene distribuito un nuovo bucket e le https://cloud.google.com/bigquery["Servizi Google Cloud BigQuery"^] sono fornite a livello di account/progetto.
...
* Se si prevede di eseguire una scansione di un file di backup alla ricerca di ransomware durante il processo di ripristino dei dati del volume (se è stata abilitata la protezione Datalock e Ransomware per i backup nel cloud), si dovranno sostenere costi aggiuntivi per il traffico in uscita dal proprio cloud provider.
...
* La terza opzione è quella di acquistare le licenze direttamente da NetApp. Consultare la <<Licenza>> sezione per i dettagli.
...
* Google Cloud Storage (solo volumi ONTAP e carichi di lavoro Kubernetes)
* StorageGRID
* ONTAP S3
* Archiviazione compatibile con S3 (solo per carichi di lavoro ONTAP Volumes)
...
* In Azure, NetApp Console utilizza un gruppo di risorse nuovo o esistente con un account di storage per il Blob container. La Console https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["blocca l'accesso pubblico ai dati del tuo blob"] per impostazione predefinita.
...
== Termini per NetApp Backup and Recovery
