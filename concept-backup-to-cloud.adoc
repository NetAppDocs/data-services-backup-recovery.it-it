...
I costi del servizio vengono pagati a NetApp e coprono sia il costo per creare backup su un archivio di oggetti o su una destinazione ONTAP sia per ripristinare volumi o file da tali backup. Paghi solo per i dati che proteggi nell'archiviazione di oggetti o in ONTAP (incluse le configurazioni SnapMirror ). I costi vengono calcolati utilizzando la capacità logica utilizzata all'origine (prima delle efficienze ONTAP ) dei volumi ONTAP sottoposti a backup su un archivio oggetti o su una destinazione ONTAP . Questa capacità è anche nota come Front-End Terabyte (FETB).
