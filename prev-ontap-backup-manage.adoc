...
. Controllare lo stato delle opzioni di protezione nella colonna "Protezione esistente". Le 3 icone stanno per "Snapshot locali", "Volumi replicati" e "Backup nell'archiviazione di oggetti".
+
...
.Passi
. Dalla scheda *Volumi*, identifica il volume su cui desideri attivare i backup, seleziona il menu Azioni image:icon-action.png["Icona Azioni"] alla fine della riga e seleziona *Attiva protezione 3-2-1*.
. Nella pagina _Definisci strategia di backup_, seleziona l'architettura di backup, quindi definisci i criteri e altri dettagli per snapshot locali, volumi replicati e file di backup. Consulta i dettagli per le opzioni di backup dai volumi iniziali attivati in questo sistema. Quindi seleziona *Avanti*.
...
+
Se hai abilitato _Datalock and Ransomware protection_ per i backup cloud nella policy di backup iniziale durante l'attivazione di NetApp Backup and Recovery per questo cluster, visualizzerai solo le altre policy che sono state configurate con DataLock. E se non hai abilitato _Datalock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery, visualizzerai solo le altre policy di backup cloud che non hanno DataLock configurato.
...
Se hai abilitato _Datalock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery per questo cluster, anche il backup on-demand sarà configurato con DataLock, e il periodo di conservazione sarà di 30 giorni. Le scansioni ransomware non sono supportate per i backup ad-hoc. link:prev-ontap-policy-object-options.html["Scopri di più sulla protezione da DataLock e Ransomware"^].
...
Questa funzionalità è disponibile solo se il backup del volume è stato creato da un sistema con ONTAP 9.11.1 o versione successiva e se hai abilitato _Datalock and Ransomware protection_ nella policy di backup-to-object.
...
.Passi
. Dal menu *Volumi*, fai clic su image:icon-actions-horizontal.gif["Più icona"] per il volume di origine e seleziona *Gestisci backup > Visualizza backup*.
+
