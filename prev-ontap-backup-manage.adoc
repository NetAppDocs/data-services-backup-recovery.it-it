...
[role="lead"]
Usa NetApp Backup and Recovery per gestire backup di snapshot, replica e storage a oggetti per i tuoi sistemi Cloud Volumes ONTAP e ONTAP on-premises. Puoi modificare le pianificazioni, abilitare o disabilitare i backup dei volumi, mettere in pausa i backup, eliminarli e annullare la registrazione del servizio.
...
. Verifica lo stato delle opzioni di protezione nella colonna "Protezione esistente". Le tre icone rappresentano "Snapshot locali", "Volumi replicati" e "Backup in storage a oggetti".
+
...
Consulta link:br-start-enable.html["Abilita NetApp Backup and Recovery per proteggere i volumi"^] per le istruzioni.
...
.Passi
. Dal menu *Volumi*, individuare il volume per il quale si desidera modificare le impostazioni dei criteri, selezionare il menu Azioni image:icon-action.png["Icona Azioni"] alla fine della riga e seleziona *Modifica strategia di backup*.
. Nella pagina _Modifica strategia di backup_, apportare modifiche ai criteri di backup esistenti per snapshot locali, volumi replicati e file di backup e selezionare *Avanti*.
+
Se hai abilitato _DataLock and Ransomware protection_ per i backup cloud nella policy di backup iniziale durante l'attivazione di NetApp Backup and Recovery per questo cluster, vedrai solo le altre policy che sono state configurate con DataLock. E se non hai abilitato _DataLock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery, vedrai solo le altre policy di backup cloud che non hanno DataLock configurato.
...
. A seconda del tipo di azione di backup che vuoi configurare, seleziona il pulsante corrispondente nel menu Bulk actions:
+
[cols="50,30"]
|===
...
|===
. Nella pagina di backup visualizzata, apportare modifiche ai criteri di backup esistenti per snapshot locali, volumi replicati o file di backup e selezionare *Salva*.
+
Se hai abilitato _DataLock and Ransomware protection_ per i backup cloud nella policy di backup iniziale durante l'attivazione di NetApp Backup and Recovery per questo cluster, vedrai solo le altre policy che sono state configurate con DataLock. E se non hai abilitato _DataLock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery, vedrai solo le altre policy di backup cloud che non hanno DataLock configurato.
...
Se hai abilitato _DataLock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery per questo cluster, anche il backup su richiesta verrà configurato con DataLock e il periodo di conservazione sarà di 30 giorni. Le scansioni ransomware non sono supportate per i backup ad hoc. link:prev-ontap-policy-object-options.html["Scopri di più sulla protezione da DataLock e Ransomware"^].

Quando crei un backup ad hoc, viene creata una snapshot sul volume di origine. Poiché questa snapshot non fa parte di una normale pianificazione delle snapshot, non verrà eliminata automaticamente. Potresti voler eliminare manualmente questa snapshot dal volume di origine una volta completato il backup. In questo modo, i blocchi relativi a questa snapshot verranno liberati. Il nome della snapshot inizierà con `cbs-snapshot-adhoc-`. https://docs.netapp.com/us-en/ontap/san-admin/delete-all-existing-snapshot-copies-volume-task.html["Scopri come eliminare uno snapshot utilizzando la CLI di ONTAP"^].
...
Questa funzionalità è disponibile solo se il backup del volume è stato creato da un sistema con ONTAP 9.11.1 o superiore e se hai abilitato _DataLock and Ransomware protection_ nella policy di backup-to-object.
...
. Seleziona image:icon-actions-horizontal.gif["Icona Azioni"] per il file di backup del volume che vuoi analizzare alla ricerca di ransomware e seleziona *Scansiona alla ricerca di ransomware*.
+
La colonna relativa alla protezione contro i ransomware indica che la scansione è in corso.
...
[NOTE]
====
* Se hai abilitato _DataLock and Ransomware protection_ nella policy iniziale durante l'attivazione di NetApp Backup and Recovery per questo cluster, tutte le policy che modifichi devono essere configurate con la stessa impostazione DataLock (Governance o Compliance). E se non hai abilitato _DataLock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery, non puoi abilitare DataLock ora.
...
+
link:prev-reference-aws-archive-storage-tiers.html["Scopri di più sull'utilizzo dell'archiviazione AWS"]. link:prev-reference-azure-archive-storage-tiers.html["Scopri di più sull'utilizzo dell'archiviazione di Azure"]. link:prev-reference-gcp-archive-storage-tiers.html["Scopri di più sull'utilizzo dell'archiviazione di Google"]. (Richiede ONTAP 9.12.1.)
...
[NOTE]
====
* Se hai abilitato _DataLock and Ransomware protection_ nella policy iniziale durante l'attivazione di NetApp Backup and Recovery per questo cluster, tutte le policy aggiuntive che crei devono essere configurate con la stessa impostazione DataLock (Governance o Compliance). E se non hai abilitato _DataLock and Ransomware protection_ durante l'attivazione di NetApp Backup and Recovery, non puoi creare nuove policy che utilizzano DataLock.
...
NOTE: Non è possibile eliminare i singoli file di backup di un volume una volta eliminata la relazione di backup. Eliminare i file di backup di un volume prima di eliminare la relazione di backup del volume.

.Passi
. Dal menu *Volumi*, seleziona image:icon-actions-horizontal.gif["Più icona"] per il volume di origine e scegli *Gestisci backup > Visualizza backup*.
+
Viene visualizzato l'elenco di tutti i file di backup.

. Seleziona un backup dall'elenco e scegli *Azioni > Elimina tutti i backup*.
. Conferma l'eliminazione inserendo il nome del volume.
. Espandere l'area *Impostazioni avanzate*.
. In *Elimina forzatamente i backup*, seleziona se eliminare forzatamente tutti i backup.
+
In rari casi potrebbe essere necessario revocare l'accesso di NetApp Backup and Recovery ai backup; ad esempio, se Backup and Recovery perde l'accesso al bucket di backup o se i backup sono DataLock-protetti ma non si desidera più conservarli. Utilizzare questa opzione per eliminare forzatamente i backup a livello di volume e di sistema.

+

CAUTION: Utilizzate questa opzione solo per una pulizia approfondita. Una volta attivata, NetApp Backup and Recovery non accederà più a questi backup (anche se gli oggetti rimangono in cloud storage). Dovrete eliminarli manualmente dal vostro cloud provider.
...
Elimina un singolo backup quando non è più necessario. Non è possibile eliminare i volumi replicati (volumi di protezione dei dati).


NOTE: Non è possibile eliminare i singoli file di backup del volume una volta eliminata la relazione di backup. È tuttavia possibile <<Elimina tutti i file di backup per un volume,eliminare tutti i file di backup per il volume>>.
...
NOTE: Non è possibile eliminare i singoli file di backup del volume una volta eliminata la relazione di backup. Tuttavia, è possibile <<Elimina tutti i file di backup per un volume,eliminare tutti i file di backup per il volume>>. Se è necessario eliminare una relazione di backup per un volume, assicurarsi di eliminare prima i backup per il volume, quindi eliminare la relazione di backup.
...
Disattivando NetApp Backup and Recovery per un sistema, le operazioni di backup e ripristino vengono sospese per tutti i suoi volumi senza eliminare i backup esistenti. Questo consente di sospendere tutte le attività di backup e ripristino per un determinato periodo di tempo senza annullare la registrazione del servizio di backup da questo sistema.
