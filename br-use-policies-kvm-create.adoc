...
[role="lead"]
In NetApp Backup and Recovery, crea i tuoi criteri di backup KVM che regolano la frequenza dei backup, l'orario di esecuzione e il numero di file di backup conservati.
...
+
*** *Abilita snapshot coerenti con la VM e coerenti con l'applicazione*: Abilita questa opzione per creare snapshot coerenti con la VM e coerenti con l'applicazione. Ciò richiede che il KVM QEMU Guest Agent sia in esecuzione sulla VM. Se il Guest Agent non è in esecuzione, gli snapshot saranno invece coerenti con il crash. Tieni presente che l'abilitazione di questa opzione può aumentare i tempi di backup e utilizzare più storage. Inoltre, la RAM attiva della VM non è inclusa negli snapshot coerenti.
