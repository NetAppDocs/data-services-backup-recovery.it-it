...
summary: Con NetApp Backup and Recovery, ripristina i dati ONTAP utilizzando Sfoglia e Ripristina. 
---
= Ripristina i dati ONTAP utilizzando Sfoglia e ripristina in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilizza Sfoglia e ripristina quando conosci il nome del volume di origine, il sistema di origine, la storage VM e la data del file di backup. Puoi ripristinare i dati ONTAP da una snapshot, un volume replicato o un backup archiviato nello storage a oggetti.
...
NOTE: La priorità di ripristino elevata non è supportata durante il ripristino dei dati dall'archiviazione Azure ai sistemi StorageGRID.
...
Puoi ripristinare i dati ONTAP da un file di backup che si trova in un sistema secondario (un volume replicato) o in storage a oggetti (un file di backup) nei seguenti sistemi.
...
| StorageGRID | Sistema ONTAP in sede  a| 
* Sistema ONTAP in sede
* Cloud Volumes ONTAP
...
| ONTAP S3 | Sistema ONTAP in sede  a| 
* Sistema ONTAP in sede
* Cloud Volumes ONTAP
...
| Archivi di oggetti compatibili con S3 | Sistema ONTAP in sede  a| 
* Sistema ONTAP in sede
* Cloud Volumes ONTAP
...
* Per Amazon S3, l'agente Console può essere distribuito in AWS o in locale
* Per Azure Blob, l'agente della Console può essere distribuito in Azure o in locale
* Per Google Cloud Storage, l'agente della console deve essere distribuito nella VPC di Google Cloud Platform
* Per StorageGRID, l'agente della Console deve essere distribuito in locale, con o senza accesso a Internet
* Per ONTAP S3, l'agente Console può essere distribuito in locale (con o senza accesso a Internet) o in un ambiente di cloud provider


Nota che i riferimenti a "sistemi ONTAP on-premises" includono FAS, AFF e ONTAP Select.
...
Quando ripristini un backup cloud su un sistema Cloud Volumes ONTAP che utilizza ONTAP 9.13.0 o versioni successive oppure su un sistema ONTAP on-premises con ONTAP 9.14.1, hai la possibilità di eseguire un'operazione di _ripristino rapido_. Il ripristino rapido è ideale per situazioni di disaster recovery in cui devi fornire accesso a un volume il prima possibile. Un ripristino rapido ripristina i metadati dal file di backup su un volume invece di ripristinare l'intero file di backup. Il ripristino rapido non è consigliato per applicazioni sensibili alle prestazioni o sensibile alla latenza e non è supportato con backup in storage archiviato.


NOTE: Il ripristino rapido è supportato per i volumi FlexGroup solo se il sistema di origine da cui è stato creato il backup cloud eseguiva ONTAP 9.12.1 o superiore. È supportato per i volumi SnapLock solo se il sistema di origine eseguiva ONTAP 9.11.0 o superiore.
...
. Seleziona la scheda *Ripristina* per visualizzare la Restore Dashboard.
...
+
La colonna *Posizione* indica se il file di backup (snapshot) è *Locale* (uno snapshot sul sistema di origine), *Secondario* (un volume replicato su un sistema ONTAP secondario) o *Storage a oggetti* (un file di backup in storage a oggetti). Scegli il file che vuoi ripristinare.

. Selezionare *Avanti*.
+
Tieni presente che se selezioni un file di backup nello storage a oggetti e la protezione ransomware è attiva per quel backup (se hai abilitato DataLock e la protezione ransomware nella policy di backup), ti verrà richiesto di eseguire una scansione ransomware aggiuntiva sul file di backup prima di ripristinare i dati. Ti consigliamo di eseguire una scansione ransomware sul file di backup. (Sosterrai costi aggiuntivi di egress dal tuo cloud provider per accedere al contenuto del file di backup.)
...
+
** Quando ripristini da Amazon S3, seleziona l'IPspace nel cluster ONTAP in cui risiederà il volume di destinazione, inserisci la access key e la secret key per l'utente che hai creato per dare al cluster ONTAP accesso al bucket S3 e, se vuoi, scegli un endpoint VPC privato per un trasferimento sicuro dei dati. Se usi un endpoint VPC privato, questo ha la precedenza su qualsiasi connessione proxy configurata.
...
. Facoltativamente, puoi abilitare *Dai priorità a questo ripristino rispetto alla coda di processi esistente* per assegnare a questa operazione di ripristino una priorità più alta. Quando abilitata, altri processi in esecuzione potrebbero essere sospesi o ritardati per consentire a questo ripristino di procedere.
. Selezionare *Ripristina*.
+
Torna alla dashboard di ripristino così puoi controllare lo stato di avanzamento dell'operazione di ripristino.
...
* Con ONTAP 9.15.1, puoi ripristinare le cartelle FlexGroup utilizzando l'opzione "Sfoglia e ripristina".
+
...
+
Se la protezione ransomware è attiva per il file di backup (se hai abilitato DataLock e la protezione ransomware nella policy di backup), ti verrà richiesto di eseguire una scansione ransomware aggiuntiva sul file di backup prima di ripristinare i dati. Ti consigliamo di eseguire una scansione ransomware sul file di backup. (Sosterrai costi di uscita aggiuntivi dal tuo cloud provider per accedere al contenuto del file di backup.)
...
+
Hai diverse opzioni per la posizione quando ripristini cartelle e file.

+
** Dopo aver scelto *Seleziona cartella di destinazione*, come mostrato sopra:
+
*** Puoi selezionare qualsiasi cartella.
*** Puoi passare il mouse su una cartella e selezionare la fine della riga per esplorare le sottocartelle, quindi selezionare una cartella.


** Se hai selezionato lo stesso sistema di destinazione e lo stesso volume della cartella o del file di origine, puoi selezionare *Mantieni percorso cartella di origine* per ripristinare la cartella o i file nella stessa cartella in cui si trovavano nella struttura di origine. Tutte le stesse cartelle e sottocartelle devono già esistere; le cartelle non vengono create. Quando ripristini i file nella posizione originale, puoi scegliere se sovrascrivere i file di origine o creare nuovi file.
