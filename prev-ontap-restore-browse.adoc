...
[role="lead"]
Con NetApp Backup and Recovery, puoi ripristinare i dati ONTAP utilizzando Sfoglia e Ripristina. Prima di ripristinare, annota il nome del volume di origine, il sistema di origine, la storage VM e la data del file di backup. Puoi ripristinare i dati ONTAP da una Snapshot, da un volume replicato o da backup archiviati nello storage a oggetti.
...
| NetApp StorageGRID | Sistema ONTAP in sede  a| 
* Sistema ONTAP in sede
* Cloud Volumes ONTAP
...
. Selezionare *Avanti*.
+
Tieni presente che se selezioni un file di backup in storage a oggetti e la protezione anti-ransomware è attiva per tale backup (se hai abilitato Datalock e la protezione anti-ransomware nella policy di backup), ti verrà richiesto di eseguire una scansione anti-ransomware aggiuntiva sul file di backup prima di ripristinare i dati. Si consiglia di eseguire una scansione anti-ransomware sul file di backup. (Sosterrai costi aggiuntivi di traffico in uscita dal tuo cloud provider per accedere al contenuto del file di backup.)
...
+
Se la protezione anti-ransomware è attiva per il file di backup (se hai abilitato Datalock e la protezione anti-ransomware nella policy di backup), ti verrà richiesto di eseguire un'ulteriore scansione anti-ransomware sul file di backup prima di ripristinare i dati. Consigliamo di eseguire una scansione anti-ransomware sul file di backup. (Sosterrai costi aggiuntivi di egress dal tuo cloud provider per accedere al contenuto del file di backup.)
