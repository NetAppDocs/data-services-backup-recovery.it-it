...
[role="lead"]
In NetApp Backup and Recovery, puoi eseguire il backup dei dati dei volumi dai sistemi ONTAP locali primari a uno storage secondario e allo storage a oggetti StorageGRID. Usa questa pagina per preparare l'ambiente e attivare i backup.
...
+
La nuova versione di NetApp Backup and Recovery, che include la possibilità di pianificare e creare snapshot e volumi replicati, oltre alla creazione di backup su storage a oggetti, richiede che tu stia utilizzando la versione 3.9.31 o superiore dell'agente Console. NetApp consiglia di usare quest'ultima release per gestire tutti i tuoi backup.
...
Per i dettagli sui requisiti di DataLock e sulla protezione dal ransomware per StorageGRID, consulta link:prev-ontap-policy-object-options.html["Opzioni di policy di backup su oggetto"].

Versioni StorageGRID supportate:: StorageGRID 10.3 e versioni successive sono supportati.
+
--
Per utilizzare DataLock & la protezione ransomware per i backup, i sistemi StorageGRID devono eseguire la versione 11.6.0.3 o successiva.
...
Per utilizzare lo storage di archiviazione, è necessario l'accesso all'indirizzo IP del nodo amministratore.
...
+
Se il cluster utilizza ONTAP 9.11.1 o versioni successive, puoi scegliere di proteggere i tuoi backup da eliminazioni e attacchi ransomware configurando _DataLock and Ransomware protection_. _DataLock_ protegge i file di backup da modifiche o eliminazioni, e _ransomware protection_ analizza i file di backup per cercare prove di un attacco ransomware nei tuoi file di backup.
