...
Per informazioni dettagliate sui requisiti di protezione da Datalock e ransomware per StorageGRID, fare riferimento a link:prev-ontap-policy-object-options.html["Opzioni di policy di backup su oggetto"].
...
+
Se il cluster utilizza ONTAP 9.11.1 o versioni successive, è possibile proteggere i backup da cancellazioni e attacchi ransomware configurando _DataLock e Ransomware protection_. _DataLock_ protegge i file di backup da modifiche o cancellazioni, e _ransomware protection_ analizza i file di backup per rilevare eventuali prove di un attacco ransomware nei file di backup.
