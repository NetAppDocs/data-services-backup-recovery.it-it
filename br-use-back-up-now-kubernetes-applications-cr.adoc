...
summary: 'Utilizza le risorse personalizzate (CR) per creare snapshot e backup on-demand di un"applicazione Kubernetes, stabilendo una baseline o proteggendo le modifiche più recenti ai dati.' 
---
= Esegui subito il backup delle applicazioni Kubernetes utilizzando risorse personalizzate in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilizza le risorse personalizzate (CR) per creare snapshot e backup on-demand di un'applicazione Kubernetes, stabilendo una baseline o proteggendo le modifiche più recenti ai dati.
...
** *spec.reclaimPolicy*: (_Opzionale_) Definisce cosa succede all'AppArchive di uno snapshot quando il Snapshot CR viene eliminato. Anche se impostato su  `Retain`, il Snapshot CR viene comunque eliminato; viene conservato solo l'AppArchive. Opzioni valide:
+
*** `Retain` (predefinito)
*** `Delete`
+
...
** *spec.dataMover*: (_Facoltativo, obbligatorio per i cluster migrati da Trident Protect_) Una stringa che indica quale strumento di backup utilizzare per l'operazione di backup. Se questo cluster è stato migrato a Backup and Recovery da Trident Protect, il valore distingue tra maiuscole e minuscole e deve essere `CBS`.
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | stringa | Tempo massimo (in secondi) di attesa affinché qualsiasi PersistentVolumeClaims (PVC) appena creato raggiunga la  `Bound` fase prima che l'operazione fallisca. | "1200" (20 minuti) 
|===