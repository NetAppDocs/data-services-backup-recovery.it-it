...
[role="lead"]
Esamina queste limitazioni prima di utilizzare NetApp Backup e ripristino per volumi ONTAP.
...
* Puoi selezionare solo un volume FlexGroup alla volta per la replica. Devi attivare i backup separatamente per ogni volume FlexGroup.
+
I volumi FlexVol non presentano tale limitazione: puoi selezionare tutti i volumi FlexVol nel tuo sistema e assegnare le stesse policy di backup.
...
* Quando si converte un volume FlexVol in un volume FlexGroup o viceversa, il volume convertito viene considerato un nuovo volume; è necessario impostare la protezione per il volume convertito dopo la conversione. È possibile ripristinare i backup creati prima della conversione in un volume diverso.
...
* Non eseguire il backup dei volumi di destinazione di SnapMirror per i quali sono abilitati gli snapshot con conservazione a lungo termine. Questi snapshot vengono creati direttamente sulla destinazione e non verranno spostati nello storage a oggetti. Questa restrizione si applica anche se la policy di SnapMirror viene successivamente modificata per disabilitare la conservazione a lungo termine.
...
+
** Le relazioni con le etichette SnapMirror `app_consistent` e  `all_source_snapshot` non verranno salvate nel cloud.
** Se crei copie locali delle snapshot sul volume di destinazione di SnapMirror (indipendentemente dalle etichette SnapMirror utilizzate), queste snapshot non verranno spostate nel cloud come backup. Devi creare una policy di snapshot con le etichette desiderate sul volume DP di origine così che NetApp Backup and Recovery possa eseguirne il backup.
...
** La policy di snapshot applicata al volume deve essere una delle policy riconosciute da NetApp Backup and Recovery, incluse quelle giornaliere, settimanali, mensili, ecc. La policy predefinita "sm_created" (utilizzata per *Mirror All Snapshots*) non è riconosciuta e il volume DP non verrà visualizzato nell'elenco dei volumi di cui puoi eseguire il backup.
** SVM-DR e il backup e ripristino dei volumi funzionano in modo completamente indipendente quando il backup viene eseguito sia dal sistema di origine che dalla destinazione. L'unica limitazione è che SVM-DR non replica la relazione SnapMirror cloud. Nello scenario di disaster recovery, quando la storage VM viene attivata nella posizione secondaria, devi aggiornare manualmente la relazione SnapMirror cloud.


* Supporto MetroCluster :
+
** Quando usi ONTAP 9.12.1 GA o versioni successive, il backup è supportato quando sei connesso al sistema primario. L'intera configurazione del backup viene trasferita al sistema secondario così i backup sul cloud continuano automaticamente dopo il passaggio. Non devi configurare il backup sul sistema secondario (in realtà, non ti è permesso farlo).
...
* Il backup ad hoc del volume tramite il pulsante *Backup Now* non è supportato sui volumi di protezione dei dati.
...
* La modalità WORM/Compliance su un archivio di oggetti è supportata su Amazon S3, Google Cloud Platform (GCP), Azure e StorageGRID in questo momento. Questa è nota come funzionalità DataLock e deve essere gestita utilizzando le impostazioni di NetApp Backup and Recovery, non tramite l'interfaccia del cloud provider.
...
* Il ripristino delle cartelle da snapshot locali non è supportato.
* Ripristino di file e cartelle in una configurazione di disaster recovery (DR): dopo un failover, i volumi su una storage VM configurata per il DR mantengono le relazioni SnapMirror preesistenti in uno stato interrotto. I tentativi di ripristinare file o cartelle che creano relazioni RST temporanee SnapMirror vengono bloccati da ONTAP perché il volume di destinazione ha già una relazione.
* Browse & Restore può ripristinare fino a 100 file singoli alla volta.
* La funzione Cerca e ripristina consente di ripristinare un file alla volta.
...
== Abilitazione dell'indicizzazione in una configurazione di ripristino di emergenza

Quando si abilita l'indicizzazione su un cluster di storage VM in una configurazione di disaster recovery (DR), viene visualizzato un errore che indica che l'operazione non è consentita su un Vserver configurato come destinazione per il DR del Vserver. Nonostante l'errore, il processo di backend imposta lo  `snapdiff-rpc-server` stato su "on" per le storage VM di origine. Dopo l'esecuzione della pianificazione della policy di DR delle storage VM, questo stato si propaga alle storage VM di destinazione.

*Soluzione alternativa*: attendere l'esecuzione della pianificazione dei criteri di ripristino d'emergenza della macchina virtuale di archiviazione dopo l'errore iniziale, quindi provare ad abilitare nuovamente l'indicizzazione.
