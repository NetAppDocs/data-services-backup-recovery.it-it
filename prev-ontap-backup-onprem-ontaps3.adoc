...
[role="lead"]
In NetApp Backup and Recovery, puoi eseguire il backup dei dati dei volumi dai sistemi ONTAP locali primari su ONTAP S3 e, facoltativamente, su un sistema ONTAP secondario per la replica. Usa questa pagina per preparare l'ambiente e attivare i backup.
...
. *Snapshot locale*: scegli una policy di snapshot esistente o creane una nuova.
+
...
+
** *Destinazione della replica*: Seleziona il sistema di destinazione e la storage VM. Facoltativamente, seleziona l'aggregato di destinazione (o gli aggregati per i volumi FlexGroup) e un prefisso o suffisso che verrà aggiunto al nome del volume replicato.
