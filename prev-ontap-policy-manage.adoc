...
[role="lead"]
In NetApp Backup and Recovery, puoi usare le policy di backup predefinite oppure creare policy personalizzate per i tuoi volumi ONTAP. Le policy controllano la frequenza dei backup, la loro pianificazione e il periodo di conservazione.
...
Quando usi la procedura guidata di attivazione per abilitare il servizio NetApp Backup and Recovery per i tuoi volumi, puoi scegliere tra le policy predefinite e qualsiasi altra policy già presente nel sistema (Cloud Volumes ONTAP o sistema ONTAP on-premises). Se vuoi usare una policy diversa da quelle esistenti, puoi crearla prima o mentre usi la procedura guidata di attivazione.
...
NetApp Backup and Recovery offre tre tipi di backup dei dati ONTAP: snapshot, repliche e backup su storage a oggetti. Le relative policy risiedono in posizioni diverse a seconda dell'architettura che usi e del tipo di backup:
...
. Nella pagina Gestione policy, seleziona *Crea policy* > *Crea policy snapshot*.
...
.Passi
. Nella pagina Gestione policy, seleziona *Crea policy* > *Crea policy di replica*.
...
.Passi
. Nella pagina Gestione criteri, seleziona *Crea criterio* > *Crea policy di backup*.
. Nella sezione Dettagli policy, specificare il nome della policy.
. Specificare le etichette SnapMirror (massimo 5) che indicano la conservazione per ciascuna etichetta.
